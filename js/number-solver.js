...
class MyNumber {
	 ...
	 * @param {number} numMiddleOnes The number of random "middle" to select.
     * @param {number} numSelections The total number of numbers to select (big, middle and little).
     * @param {number[]} bigOnes Array of the big numbers to choose from.
	 * @param {number[]} middleOnes Array of the middle numbers to choose from.
    ...
    constructor(target, numBigOnes, numMiddleOnes, numSelections, bigOnes, middleOnes, littleOnes) {
        if (target == null) target = this.randomMain(0,1000);
		...
		if (middleOnes == null) middleOnes = this.standardMiddleOnes();
		...
		for (let i = 0; i < numMiddleOnes; ++i) {
            this.values.push(middleOnes.pop());
        }
        const numLittleOnes = numSelections - numBigOnes - numMiddleOnes;
	...
	/** Return an array of the standard middle numbers. */
    standardMiddleOnes() {
        return [10, 15, 20];
    }
    ...
    solve() {
		...
		solns.all = solns.all.filter((v,i,a)=>a.findIndex(v2=>(v2.simpleExpression===v.simpleExpression))===i);
		
		solns.bestSort.sort((v1, v2) => v1.compareTo(v2, this.target));
		solns.bestSort = solns.bestSort.filter((v,i,a)=>a.findIndex(v2=>(v2.simpleExpression===v.simpleExpression))===i).slice(0,5);
        return solns;
    }
